...
public class DTNHost implements Comparable<DTNHost> {
	...
	/**
	 * Force a one-way connection event
	 */
	public void forceOneWayConnection(DTNHost anotherHost, String interfaceId,
			boolean up) {
		NetworkInterface ni;
		NetworkInterface no;
		...
		if (up) {
			ni.createOneWayConnection(no);
		} else {
			ni.destroyOneWayConnection(no);
		}
	}
	...
	public int receiveMessage(Message m, DTNHost from) {
		...
		if (retVal == MessageRouter.RCV_OK) {
			...
			//start transfer transimit the message
//			String tmp_str = String.format("[src:%d dst:%d] M%s transmit from %s to %s", m.getFrom().getAddress(), m.getTo().getAddress(),
//					m.getId(), from.toString(), this.toString());
//			if (m.getTo().getAddress() == this.getAddress()) {
//				tmp_str = String.format("%s, M%s delivered", tmp_str, m.getId());
