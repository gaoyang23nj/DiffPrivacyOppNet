...
//			String tmp_str = m.toString() + " via " + con.toString() + " res:" + retVal;
...
//		if (!messages.isEmpty()) {
//			String tmp_str = super.getHost().toString() + " " + messages.size() + " [ " ;
//			
//			for(int i=0; i<messages.size(); i++) {
//				tmp_str = tmp_str + messages.get(i).toString() + ",";
//			}
//			tmp_str = tmp_str + "]";
//			System.out.println(tmp_str);
//		}
			...
			if (con.isMessageTransferred()) {
				if (con.getMessage() != null) {
					...
					if (m.getTo().getAddress() == to.getAddress()) {
						String tmp_str = String.format("[src:%d dst:%d] M%s transmit from %s to %s", m.getFrom().getAddress(), 
								m.getTo().getAddress(), m.getId(), from.toString(), to.toString());
