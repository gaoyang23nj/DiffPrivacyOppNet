...
class RoutingRTPMSpdUp_Theory(object):
    ...
    def __cal_convolprob(self, runningtime, onepath):
        ...
        ############################## method_2 更符合积分理论
        # n^k 可以通过深度优先 或者 广度优先 搜索解决
        # 从第0层 开始 遍历各个位置(0,...)
        # tmp_prob = 1.
        # sum_prob = self.__research_next(targetmatrix, matrix_size, 0, 0, tmp_prob)
        ############################## method_3 相乘方法
        sum_prob = self.__mul_sum(targetmatrix)
        return sum_prob

    def __mul_sum(self, targetmatrix):
        return targetmatrix.sum(axis=1).prod()

    def __research_next(self, targetmatrix, matrix_size, tier, begin, tmp_prob):
        # 达到最后一层
        ...
        for i in range(begin, matrix_size[1]):
            ...
            tmp_res = tmp_res + self.__research_next(targetmatrix, matrix_size, tier+1, i+1, tmp_prob)
        return tmp_res
