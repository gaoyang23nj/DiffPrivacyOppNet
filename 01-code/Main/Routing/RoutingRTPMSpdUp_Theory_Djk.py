...
class RoutingRTPMSpdUp_Theory_Djk(object):
    # 可以把{p}配置成同样的seg个数
    # def __init__(self, node_id, num_of_nodes, min_time, max_time, input_list_weather, max_ttl, lap_noise_scale, theconfig):
    def __init__(self, node_id, num_of_nodes, min_time, max_time, input_list_weather, max_ttl):
    ...
    def __process_pdf_intraday_withGMM(self):
        ...
        for b_id in range(self.num_of_nodes):
            ...
            tmp_lap_probs_holiday[b_id, :] = probs_h
            tmp_lap_probs_workday[b_id, :] = probs_w
    ...
    def __writePandp(self, runningtime):
        if runningtime.tm_mon != 7:
            return
        filename = 'PandpforOne/BigP_holiday_node_{}_day_{}.csv'.format(self.node_id, runningtime.tm_mday)
        f = open(filename, 'w+')
        f.write('node:{} time:{} '.format(self.node_id, runningtime.tm_yday))
        for i in range(len(self.all_P_holiday)):
            f.write('from_node:{}\n'.format(i))
            tmp = '{}'.format(self.all_P_holiday[i][0])
            for j in range(1, len(self.all_P_holiday[i])):
                tmp = tmp + ',{}'.format(self.all_P_holiday[i][j])
            f.write(tmp+'\n')
        f.close()

        filename = 'PandpforOne/BigP_workday_node_{}_day_{}.csv'.format(self.node_id, runningtime.tm_mday)
        f = open(filename, 'w+')
        f.write('node:{} time:{}'.format(self.node_id, runningtime.tm_yday))
        for i in range(len(self.all_P_workday)):
            f.write('from_node:{}\n'.format(i))
            tmp = '{}'.format(self.all_P_workday[i][0])
            for j in range(1, len(self.all_P_workday[i])):
                tmp = tmp + ',{}'.format(self.all_P_workday[i][j])
            f.write(tmp + '\n')
        f.close()

        filename = 'PandpforOne/smallp_holiday_node_{}_day_{}.csv'.format(self.node_id, runningtime.tm_mday)
        f = open(filename, 'w+')
        f.write('node:{} time:{}'.format(self.node_id, runningtime.tm_yday))
        for i in range(len(self.all_pdf_holiday)):
            f.write('from_node:{}\n'.format(i))
            for j in range(len(self.all_pdf_holiday[i])):
                f.write('to_node:{}\n'.format(j))
                tmp = '{}'.format(self.all_pdf_holiday[i][j][0])
                for k in range(1, len(self.all_pdf_holiday[i][j])):
                    tmp = tmp + ',{}'.format(self.all_pdf_holiday[i][j][k])
                f.write(tmp + '\n')
        f.close()

        filename = 'PandpforOne/smallp_workday_node_{}_day_{}.csv'.format(self.node_id, runningtime.tm_mday)
        f = open(filename, 'w+')
        f.write('node:{} time:{}'.format(self.node_id, runningtime.tm_yday))
        for i in range(len(self.all_pdf_workday)):
            f.write('from_node:{}\n'.format(i))
            for j in range(len(self.all_pdf_workday[i])):
                f.write('to_node:{}\n'.format(j))
                tmp = '{}'.format(self.all_pdf_workday[i][j][0])
                for k in range(1, len(self.all_pdf_workday[i][j])):
                    tmp = tmp + ',{}'.format(self.all_pdf_workday[i][j][k])
                f.write(tmp + '\n')
        f.close()
    ...
    def notify_new_day(self, runningtime):
        ...
        # 更新处理时间
        # Record P and p for conducting in `the one'
        isRecordPandp = True
        if isRecordPandp:
            self.__writePandp(runningtime)
