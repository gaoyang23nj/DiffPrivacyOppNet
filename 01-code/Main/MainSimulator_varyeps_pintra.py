...
class Simulator(object):
    def __init__(self, num_station, enco_file, pktgen_freq, ttl, result_file_path, eps_tunple):
        print(eps_tunple)
        self.eps_tunple = eps_tunple
        # TTL为1天
        self.ttl = ttl
    ...
    def init_scenario(self):
        self.scenaDict = {}
        # list_scena = self.init_scenario_Lap_Pinter()
        # list_scena = self.init_scenario_Lap_Pintra()
        # list_scena = self.init_scenario_DODP_Pinter()
        # list_scena = self.init_scenario_DODP_Pintra()
        list_scena = self.init_scenario_eps()
        return list_scena

    def init_scenario_eps(self):
        index = -1

        # ===============================场景1 RTPM_Theory_Djk_Lap ===================================
        index += 1
        ...
        tmpscenario = DTNScenario_RTPMSpdUp_Theory_Djk_LapDP_Pp(tmp_senario_name, self.MAX_NODE_NUM, 20000,
                                                                ...
                                                                self.max_ttl, self.eps_tunple)
        self.scenaDict.update({tmp_senario_name: tmpscenario})

        # ===============================场景2 RTPM_Theory_Djk_OpDP ===================================
        index += 1
        ...
        # ===============================场景4 RTPM_Theory_Djk_StairDP ===================================
        index += 1
        tmp_senario_name = 'scenario' + str(index) + 'DTNScenario_RTPMSpdUp_Theory_Djk_StairDP'
        # 第一个分量加在{p}天内上，第二个分量加在P天间上
        # 输入参数是 laplace分布的lambda参数
...
if __name__ == "__main__":
    ...
    for kkk in range(1):
        ...
        # result file path
        short_time = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
        result_file_path = "res_oppnet_varyeps_pintra_qiaobeidata_" + short_time + ".csv"
        ...
        # list_ttl = [1,2,3,4]
        eps_list = [0.2, 0.4, 0.6, 0.8, 1., 1.2]
        list_eps = []
        if isepsPinter:
            for eps in eps_list:
                list_eps.append((2, 1./eps))
        else:
            for eps in eps_list:
                list_eps.append((2./eps, 1.))
        ttl = 5
        for eps_tunple in list_eps:
            print(EncoHistDir, genpkt_freq)
            t_start = datetime.datetime.now()
            theSimulator = Simulator(num_station, EncoHistDir, genpkt_freq, ttl, result_file_path, eps_tunple)
