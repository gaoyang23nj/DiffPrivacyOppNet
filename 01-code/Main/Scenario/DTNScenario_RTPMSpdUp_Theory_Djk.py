...
from Main.Routing.RoutingRTPMSpdUp_Theory_Djk import RoutingRTPMSpdUp_Theory_Djk
...
class DTNScenario_RTPMSpdUp_Theory_Djk(object):
    ...
    def __init__(self, scenarioname, num_of_nodes, buffer_size, min_time, max_time, max_ttl):
        # print('memo')
        ...
        for node_id in range(num_of_nodes):
            # 用来配置同样的seg
            tmpRouter = RoutingRTPMSpdUp_Theory_Djk(node_id, num_of_nodes, min_time, max_time,
                                                             self.list_weather, self.max_ttl)
