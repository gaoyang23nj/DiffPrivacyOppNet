...
from Main.Routing.RoutingRTPMSpdUp_Theory_Djk_LapDP_Pp import RoutingRTPMSpdUp_Theory_Djk_LapDP_Pp
...
class DTNScenario_RTPMSpdUp_Theory_Djk_LapDP_Pp(object):
    ...
    def __init__(self, scenarioname, num_of_nodes, buffer_size, min_time, max_time, max_ttl, lap_noise_scale):
        ...
        for node_id in range(num_of_nodes):
            # 用来配置同样的seg
            # tmpRouter = RoutingRTPMSpdUp_Theory_Djk_LapDP_Pp(node_id, num_of_nodes, min_time, max_time,
    ...
    def sendpkt(self, runningtime, a_id, b_id):
        ...
        for tmp_pkt in totran_pktlist:
            ...
            isNeedtoFwd = False
            # 判断是否有必要从a转发给b
            P_a_dst, path_a = self.listRouter[a_id].get_values_before_up(runningtime, tmp_pkt.gentime, tmp_pkt.dst_id)
            ...
            if len(path_a) > 0 and path_a[1] == b_id:
                ...
                P_b_dst, path_b = self.listRouter[b_id].get_values_before_up(runningtime, tmp_pkt.gentime, tmp_pkt.dst_id)
    ...
    def print_res(self, listgenpkt):
        ...
        config = {'lap_noise_scale': self.lap}
